-- ============================================================================
-- FIXED: Add Missing Columns (Handles Existing Tables)
-- ============================================================================
-- This version properly handles tables that already exist
...
BEGIN
  ...
  -- ===========================================================================
  -- STEP 4: Fix coupons table (add missing columns if table exists)
  -- ===========================================================================
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables 
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
      ...
      ALTER TABLE public.coupons ADD COLUMN max_uses_per_user INTEGER DEFAULT 1 CHECK (max_uses_per_user > 0);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'coupons' AND column_name = 'times_used') THEN
      ALTER TABLE public.coupons ADD COLUMN times_used INTEGER DEFAULT 0 CHECK (times_used >= 0);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'coupons' AND column_name = 'max_discount_amount') THEN
      ALTER TABLE public.coupons ADD COLUMN max_discount_amount INTEGER CHECK (max_discount_amount IS NULL OR max_discount_amount >= 0);
    END IF;
...
-- ===========================================================================
-- STEP 7: Insert/Update default coupons
-- ===========================================================================
...
-- ===========================================================================
-- STEP 8: Create/Replace validation function
-- ===========================================================================
...
-- ===========================================================================
-- STEP 9: Update product categories
-- ===========================================================================
...
SELECT '📦 Coupons:' as info, code, discount_type, is_active 
FROM public.coupons 
ORDER BY code;
...
WHERE table_name IN ('orders', 'cart_items', 'products', 'coupons')
  AND column_name IN (
    'coupon_id', 'subtotal', 'delivery_charge', 'discount_amount', 'gst_amount',
    'razorpay_order_id', 'razorpay_payment_id', 'razorpay_signature',
    'selected_variant', 'original_price', 'stock_quantity', 'max_uses_per_user'
  );

-- ===========================================================================
-- DONE! Your database is now ready for checkout with Razorpay & coupons
-- ===========================================================================
